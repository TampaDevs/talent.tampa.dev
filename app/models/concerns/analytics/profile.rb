module Analytics
  module Profile
    ...
    def ap_types
      profiles = []
      profiles << :user
      profiles << :developer if try(:developer).present?
      profiles << :business if try(:business).present?
      profiles
    end
    ...
    def analytics_profile
      ...
      if is_a?(User)
        if ap_types.include?(:developer)
          profile[:ap_developer_hashid] = try(:developer).try(:hashid)
        end

        if ap_types.include?(:business)
          profile[:ap_business_id] = try(:business).try(:id)
        end
      elsif is_a?(Developer)
        profile[:ap_developer_hashid] = try(:hashid)
      elsif is_a?(Business)
        profile[:ap_business_id] = try(:id)
      end
