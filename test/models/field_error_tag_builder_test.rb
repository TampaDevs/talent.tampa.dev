...
class FieldErrorTagBuilderTest < ActiveSupport::TestCase
  ...
  test "replace only text color class" do
    html_tag = %(<label class="bg-blue-200 font-medium text-blue-200">Test Label</label>)
    builder = FieldErrorTagBuilder.new(html_tag, @instance)

    assert_equal html_tag.length - builder.color_free_tag.length, "text-blue-200".length
    assert_not builder.color_free_tag.include?("text-blue-200")
  end

  test "grabs correct closing tag index" do
    html_tag = %(<label class="bg-blue-200 font-medium">Test Label</label>)
    builder = FieldErrorTagBuilder.new(html_tag, @instance)
